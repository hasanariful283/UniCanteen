...
type Conversation = {
  ...
  messages?: Array<{ id: string; content: string; createdAt: string; sender: User }>;
...
function MessagesPageInner() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch(`/api/messages/conversations/${selectedId}/messages`, { cache: "no-store" });
        const data = await res.json();
        ...
        {loading ? (
          ...
          <ul>
            {conversations.map((c) => {
              ...
              const title = c.participants.map(p => p.user?.name || p.user?.email || p.userId).join(", ");
              return (
                <li key={c.id}>
                  <button
                  ...
                  >
                    ...
                    {last ? (
                      <div className="text-xs text-muted-foreground line-clamp-1">{last.content}</div>
                    ) : (
          ...
          <div className="flex-1 flex flex-col">
            <header className="p-4 border-b">
              <div className="font-semibold">
                {selectedConversation.participants.map(p => p.user?.name || p.user?.email || p.userId).join(", ")}
              </div>
            </header>
            <section className="flex-1 overflow-y-auto p-4 space-y-2">
              {messages.map((m) => {
                const isMe = m?.sender?.id === user?.id || m?.sender?.id === "me"; // optimistic uses 'me'
                return (
            ...
            <footer className="p-3 border-t flex gap-2 mb-5">
              ...
              />
              <button onClick={send} disabled={sending || !input.trim()} className="border rounded px-3 py-2">
                Send
              </button>
            </footer>
          </div>
